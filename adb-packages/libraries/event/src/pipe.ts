 ...
 */
export function pipe<TSource, TNew>(source: Event<TSource>, transform: (value: TSource) => TNew): Event<TNew> {
  return (listener: (value: TNew, ...args: unknown[]) => void, thisArg?: unknown, ...args: unknown[]) => {
    return source((value) => {
