...
describe('CodecOptions', () => {
  describe('setInt', () => {
    it('should omit type', () => {
      assert.strictEqual(new CodecOptions().setInt('profile', 42).toOptionValue(), 'profile=42');
    });
  });
  ...
  describe('setFloat', () => {
    it('should add type', () => {
      assert.strictEqual(
        new CodecOptions().setFloat('max-fps-to-encoder', 88.88).toOptionValue(),
  ...
  describe('setLong', () => {
    it('should add type', () => {
      assert.strictEqual(
        new CodecOptions().setLong('repeat-previous-frame-after', 100n).toOptionValue(),
  ...
  describe('setString', () => {
    it('should add type', () => {
      assert.strictEqual(new CodecOptions().setString('key', 'value').toOptionValue(), 'key:string=value');
    });

    it('should escape commas', () => {
      assert.strictEqual(new CodecOptions().setString('key', 'a,b,c').toOptionValue(), 'key:string=a\\,b\\,c');
    });
  });
