...
export class AdbScrcpyExitedError extends Error {
  ...
  static override [Symbol.hasInstance](value: unknown) {
    return !!(value as AdbScrcpyExitedError | undefined)?.[AdbScrcpyExitedErrorBrand];
  }
...
export type AdbScrcpyAudioStreamMetadata =
  ScrcpyAudioStreamDisabledMetadata | ScrcpyAudioStreamErroredMetadata | AdbScrcpyAudioStreamSuccessMetadata;
...
export class AdbScrcpyClient<TOptions extends AdbScrcpyOptions<object>> {
  ...
  static async start<TOptions extends AdbScrcpyOptions<Pick<ScrcpyOptions1_15.Init, 'tunnelForward'>>>(
    adb: Adb,
    path: string,
    options: TOptions
  ...
  constructor({ options, process, output, videoStream, audioStream, controlStream }: AdbScrcpyClientInit<TOptions>) {
