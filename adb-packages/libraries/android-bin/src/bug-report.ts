...
export class BugReport {
  ...
  static async queryCapabilities(adb: Adb): Promise<BugReport> {
    ...
    const result = await adb.subprocess.shellProtocol.spawn(['bugreportz', '-v']).wait().toString();
  ...
  bugReportZStream(): ReadableStream<Uint8Array> {
    return new PushReadableStream(async (controller) => {
      const process = await this.#adb.subprocess.shellProtocol!.spawn(['bugreportz', '-s'], controller.abortSignal);
      process.stdout
        .pipeTo(
          new WritableStream({
            async write(chunk) {
