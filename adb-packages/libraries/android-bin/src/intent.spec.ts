...
describe('Intent', () => {
  describe('serializeIntent', () => {
    it('should serialize intent action', () => {
      assert.deepStrictEqual(serializeIntent({ action: 'test_action' }), ['-a', 'test_action']);
    });

    it('should serialize intent categories', () => {
      assert.deepStrictEqual(serializeIntent({ categories: ['category_1', 'category_2'] }), [
        '-c',
        'category_1',
        '-c',
        'category_2'
    ...
    it('should serialize intent package', () => {
      assert.deepStrictEqual(serializeIntent({ package: 'package_1' }), ['-p', 'package_1']);
    });
    ...
    it('should serialize intent data', () => {
      assert.deepStrictEqual(serializeIntent({ data: 'data_1' }), ['-d', 'data_1']);
    });
    ...
    describe('extras', () => {
      ...
      it('should serialize component name', () => {
        assert.deepStrictEqual(
          ...
          ['--ecn', 'key1', 'com.example.package_1/com.example.component_1']
        );
      });
