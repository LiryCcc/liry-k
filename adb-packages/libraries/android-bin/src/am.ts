...
const START_ACTIVITY_OPTIONS_MAP: Partial<Record<keyof ActivityManager.StartActivityOptions, string>> = {
  displayId: '--display',
...
export class ActivityManager {
  ...
  async startActivity(options: ActivityManager.StartActivityOptions): Promise<void> {
    ...
    const command = buildCommand([ActivityManager.ServiceName, 'start', '-W'], options, START_ACTIVITY_OPTIONS_MAP);
  ...
  async broadcast(intent: Intent, options?: { user?: SingleUserOrAll; receiverPermission?: string }) {
    ...
    if (options) {
      ...
      if (options.receiverPermission) {
        command.push('--receiver-permission', options.receiverPermission);
      }
    }
    ...
    if (this.#apiLevel !== undefined && this.#apiLevel <= 25) {
      command[0] = ActivityManager.CommandName;
      await this.#adb.subprocess.noneProtocol.spawn(command.map(escapeArg)).wait();
    } else {
