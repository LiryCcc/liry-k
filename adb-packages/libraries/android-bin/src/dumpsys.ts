...
const BatteryDumpFields: Record<
  string,
  {
    ...
    field: keyof DumpSys.BatteryInfo;
  }
> = {
...
export class DumpSys {
  ...
  async diskStats() {
    const result = await this.#adb.subprocess.noneProtocol.spawn(['dumpsys', 'diskstats']).wait().toString();
  ...
  async battery(): Promise<DumpSys.BatteryInfo> {
    const result = await this.#adb.subprocess.noneProtocol.spawn(['dumpsys', 'battery']).wait().toString();

    const info: DumpSys.BatteryInfo = {
      acPowered: false,
      usbPowered: false,
      wirelessPowered: false,
      dockPowered: false,
      status: DumpSys.BatteryStatus.Unknown,
      health: DumpSys.BatteryHealth.Unknown
    };
...
export namespace DumpSys {
  ...
  export type BatteryStatus = (typeof Status)[keyof typeof Status];
  export const BatteryHealth = Health;
  export type BatteryHealth = (typeof Health)[keyof typeof Health];
