...
function getNumberType(type: 'int' | Intent.Number['type']) {
  switch (type) {
    case 'int':
      return '--ei';
    case 'long':
...
function serializeExtraArray(array: Intent.NumberArray | Intent.StringArray): [type: string, value: string] {
  let type: string;
  let value: string;

  if (array.itemType === 'string') {
    type = '--es';
    value = (array as Intent.StringArray).value.map((item) => item.replaceAll(',', '\\,')).join(',');
  } else {
...
function serializeString(result: string[], key: string, value: string | undefined) {
...
export function serializeIntent(intent: Intent) {
      ...
      switch (typeof value) {
        ...
        case 'object':
          ...
          if ('packageName' in value) {
            result.push('--ecn', key, value.packageName + '/' + value.className);
            break;
          }
          ...
          switch (value.type) {
            ...
            case 'arrayList':
              {
                const [type, valueString] = serializeExtraArray(value);
                result.push(type, key, valueString);
              }
              break;
            default:
              result.push(getNumberType(value.type), key, value.value.toString());
              break;
          }
          break;
        case 'number':
          if (value % 1 !== 0) {
            throw new Error(`Extra \`${key}\` is not an integer, must use \`Intent.ExtraNumber\` type instead`);
          }
  ...
  if (intent.component) {
    result.push('-n', intent.component.packageName + '/' + intent.component.className);
  }
