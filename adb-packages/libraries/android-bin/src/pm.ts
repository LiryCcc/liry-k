...
function option<T>(name: string, minApiLevel?: number, maxApiLevel?: number): OptionDefinition<T> {
  return {
    name,
    minApiLevel,
    maxApiLevel
...
const InstallOptionsDefinitions = {
  ...
  installLocation: option<PackageManagerInstallLocation>('--install-location', 24),
  ...
  bypassLowTargetSdkBlock: option<boolean>('--bypass-low-target-sdk-block', 34),
  ...
  disableAutoInstallDependencies: option<boolean>('--disable-auto-install-dependencies', 36)
} as const;

const ListPackagesOptionsMap: Record<keyof PackageManager.ListPackagesOptions, string> = {
  listDisabled: '-d',
  listEnabled: '-e',
  showSourceDir: '-f',
  showInstaller: '-i',
...
const UninstallOptionsMap: Record<keyof PackageManager.UninstallOptions, string> = {
  keepData: '-k',
  user: '--user',
...
const ResolveActivityOptionsMap: Partial<Record<keyof PackageManager.ResolveActivityOptions, string>> = {
  user: '--user'
};
...
function buildInstallCommand(
  ...
  for (const [key, value] of Object.entries(options)) {
    ...
    const option = InstallOptionsDefinitions[key as keyof PackageManager.InstallOptions];
    ...
    if (apiLevel !== undefined) {
      if (option.minApiLevel !== undefined && apiLevel < option.minApiLevel) {
        continue;
      }
      if (option.maxApiLevel !== undefined && apiLevel > option.maxApiLevel) {
        continue;
      }
    }
    ...
    switch (typeof value) {
      ...
      case 'string':
        ...
        break;
      default:
        throw new Error(`Unsupported type for option ${key}: ${typeof value}`);
    }
  }
...
export class PackageManager {
   ...
   */
  async install(apks: readonly string[], options?: PackageManager.InstallOptions): Promise<void> {
  ...
  async *listPackages(
    ...
    const command = buildCommand(['package', 'list', 'packages'], options, ListPackagesOptionsMap);
  ...
  async getPackageSources(
    ...
    if (this.#apiLevel !== undefined && this.#apiLevel <= 27) {
      command[0] = PackageManager.CommandName;
      process = await this.#adb.subprocess.noneProtocol.spawn(command.map(escapeArg));
    } else {
    ...
    for await (const line of lines) {
      ...
      result.push(line.substring(PackageManager.PackageListItemPrefix.length));
    }

    return result;
  }

  async uninstall(packageName: string, options?: Optional<PackageManager.UninstallOptions>): Promise<void> {
    const command = buildCommand([PackageManager.ServiceName, 'uninstall'], options, UninstallOptionsMap);
  ...
  async resolveActivity(options: PackageManager.ResolveActivityOptions): Promise<string | undefined> {
   ...
   */
  async sessionCreate(options?: PackageManager.InstallOptions): Promise<number> {
    const command = buildInstallCommand('install-create', options, this.#apiLevel);
  ...
  async sessionAddSplit(sessionId: number, splitName: string, path: string): Promise<void> {
    const command: string[] = [PackageManager.CommandName, 'install-write', sessionId.toString(), splitName, path];

    // Similar to `install`, must use `adb.subprocess` so it can read `path`
    const process = await this.#adb.subprocess.noneProtocol.spawn(command.map(escapeArg));
  ...
  async sessionAddSplitStream(
    ...
    const process = await this.#cmd.spawn(command);
    await Promise.all([stream.pipeTo(process.stdin), this.checkResult(process.output)]);
  }
  ...
  async sessionCommit(sessionId: number): Promise<void> {
    const command: string[] = [PackageManager.ServiceName, 'install-commit', sessionId.toString()];
  ...
  async sessionAbandon(sessionId: number): Promise<void> {
    const command: string[] = [PackageManager.ServiceName, 'install-abandon', sessionId.toString()];
    const process = await this.#cmd.spawn(command);
    await this.checkResult(process.output);
  }
}
...
export namespace PackageManager {
  export type InstallOptions = {
    [K in keyof typeof InstallOptionsDefinitions]?: (typeof InstallOptionsDefinitions)[K]['type'] | undefined;
  };
