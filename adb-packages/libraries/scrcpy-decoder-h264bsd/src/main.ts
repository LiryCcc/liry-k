...
export class H264BsdDecoder implements ScrcpyVideoDecoder {
  ...
  async #createRemoteDecoder(webGl: boolean) {
    ...
    return new Constructor(Comlink.transfer(canvas, [canvas]), webGl, Comlink.proxy(this.#handleSizeChange));
  }
  ...
  #handleSizeChange = ({ width, height }: { width: number; height: number }) => {
  ...
  async #decode() {
    ...
    try {
      ...
      let chunk: (ScrcpyVideoDecoderPauseController.Output & { type: 'data' }) | undefined;
