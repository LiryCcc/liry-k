...
import type { AddressInfo, IpcSocketConnectOpts, SocketConnectOpts, TcpSocketConnectOpts } from 'node:net';
...
const SocketTypes: Record<string, (spec: string) => SocketConnectOpts> = {
  ...
  local: (spec) => {
    if (platform() === 'win32') {
      throw new Error('socket type local is unavailable on this platform');
    }
    return { path: spec };
  },
  localabstract: () => {
  ...
  localfilesystem: (spec) => {
    if (platform() === 'win32') {
      throw new Error('socket type localfilesystem is unavailable on this platform');
    }
    return { path: spec };
  }
};
...
function getServerPort() {
  ...
  if (port === undefined || port <= 0 || port > 65535) {
    throw new Error(`$ANDROID_ADB_SERVER_PORT must be a positive number less than 65535: got "${value}"`);
  }
  return port;
}
...
export class AdbServerNodeTcpConnector implements AdbServerClient.ServerConnector {
  ...
  async addReverseTunnel(handler: Adb.IncomingSocketHandler, address?: string): Promise<string> {
    ...
    if (address) {
      if (address.startsWith('tcp:')) {
        ...
        if (!spec.port) {
          throw new Error(`Invalid destination port: '${address.substring(4)}'`);
        }
