...
describe('Consumable', () => {
  it('should export all symbols', () => {
    ...
    assert(!!Consumable.WrapWritableStream, 'WrapWritableStream should be define');

    assert(!!Consumable.ReadableStream, 'ReadableStream should be define');

    assert(!!Consumable.WrapByteReadableStream, 'WrapByteReadableStream should be define');
  });
});
