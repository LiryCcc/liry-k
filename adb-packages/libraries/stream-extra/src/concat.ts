...
export class AccumulateStream<Input, Output, Accumulated = Output> implements ReadableWritablePair<Output, Input> {
  #current: Accumulated;
  ...
  #readable = new ReadableStream<Output>(
    ...
    { highWaterMark: 0 }
  ) as ReadableStream<Output> & Omit<Promise<Output>, typeof Symbol.toStringTag>;
  get readable() {
...
export function toUint8Array(buffer: ArrayBufferLike | ArrayBufferView<ArrayBufferLike>): Uint8Array {
...
export function concatBuffers(buffers: readonly (ArrayBufferLike | ArrayBufferView<ArrayBufferLike>)[]): Uint8Array {
 ...
 */
export class ConcatBufferStream extends AccumulateStream<Uint8Array, Uint8Array, Uint8Array[]> {
  constructor() {
    super(
      [],
      (chunk, current) => {
