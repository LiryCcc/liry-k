...
export type ConcatFields<T extends Struct<StructFields, Record<PropertyKey, unknown> | undefined, unknown>[]> = As<
  UnionToIntersection<T[number]['fields']>,
  StructFields
>;

type ConcatFieldValues<T extends Struct<StructFields, Record<PropertyKey, unknown> | undefined, unknown>[]> =
...
export type ConcatExtras<T extends Struct<StructFields, Record<PropertyKey, unknown> | undefined, unknown>[]> = As<
...
export function concat<
  T extends Struct<StructFields, Record<PropertyKey, unknown> | undefined, unknown>[],
...
>(
  ...
  return struct(structs.reduce((fields, struct) => Object.assign(fields, struct.fields), {}) as never, {
    littleEndian: options.littleEndian,
    postDeserialize: options.postDeserialize,
    extra: structs.reduce(
      (extras, struct) => Object.defineProperties(extras, Object.getOwnPropertyDescriptors(struct.extra)),
      {}
    ) as never
  });
}
