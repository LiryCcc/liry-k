...
describe('Struct', () => {
  ...
  it('use struct as field', () => {
    const B = struct({ foo: struct({ bar: u8 }, { littleEndian: true }), baz: u16 }, { littleEndian: true });
    assert.deepStrictEqual(B.serialize({ foo: { bar: 10 }, baz: 20 }), new Uint8Array([10, 20, 0]));
  ...
  describe('type', () => {
    ...
    it('should be `byob` if all fields are byob', () => {
      ...
      const C = struct({ a: u8, b: u16, c: string(10) }, { littleEndian: true });
    ...
    it('should be `default` if any field is default', () => {
      ...
      const B = struct({ a: string(u32), b: u32 }, { littleEndian: true });
