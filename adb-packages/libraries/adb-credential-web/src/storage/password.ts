...
async function deriveAesKey(password: string, saltOrLength: Uint8Array<ArrayBuffer> | number, iterations: number) {
  ...
  async *load(): AsyncGenerator<MaybeError<TangoKey>, void, void> {
    for await (const result of this.#storage.load()) {
      ...
      try {
        ...
        yield e instanceof Error ? e : new Error(String(e), { cause: e });
      }
    }
  }
}
...
export namespace TangoPasswordProtectedStorage {
  export type RequestPassword = (reason: 'save' | 'load', name: string | undefined) => MaybePromiseLike<string>;
