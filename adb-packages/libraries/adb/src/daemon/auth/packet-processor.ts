...
export class AdbDaemonDefaultAuthProcessor implements AdbDaemonAuthProcessor {
  ...
  #iterator:
    Iterator<MaybeError<AdbPrivateKey>, void, void> | AsyncIterator<MaybeError<AdbPrivateKey>, void, void> | undefined;
  ...
  async process(packet: AdbPacketData): Promise<AdbPacketData> {
    ...
    const nameBuffer = key.name?.length ? encodeUtf8(key.name) : EmptyUint8Array;
