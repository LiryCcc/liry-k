...
export class AdbPacketDispatcher implements Closeable {
  ...
  readonly #incomingSocketHandlers = new Map<string, Adb.IncomingSocketHandler>();
    ...
    connection.readable
      .pipeTo(
        new WritableStream<AdbPacketData>({
          write: async (packet: AdbPacketData, controller: WritableStreamDefaultController) => {
  ...
  #handleOkay(packet: AdbPacketData) {
    ...
    this.sendPacket(AdbCommand.Close, packet.arg1, packet.arg0, EmptyUint8Array).catch(() => {});
  }
