...
function parseDeviceLineItem<T extends string = string, Optional extends boolean = false>(
  line: string,
  end: number,
  key: string,
  optional?: Optional
...
export class AdbServerClient {
  ...
  static parseDeviceList(
    value: string,
    includeStates: readonly AdbServerClient.ConnectionState[] = ['device', 'unauthorized']
    ...
    for (const line of value.split('\n')) {
      ...
      // transport_id:123
      const [newEnd0, value] = parseDeviceLineItem(line, end, KeyTransportId);
      end = newEnd0;
      const transportId = BigInt(value);

      // device:abc (optional)
      const [newEnd1, device] = parseDeviceLineItem(line, end, KeyDevice, true);
      end = newEnd1;

      // model:abc (optional)
      const [newEnd2, model] = parseDeviceLineItem(line, end, KeyModel, true);
      end = newEnd2;

      // product:abc (optional)
      const [newEnd3, product] = parseDeviceLineItem(line, end, KeyProduct, true);
      end = newEnd3;
   ...
   */
  async trackDevices(options?: AdbServerDeviceObserverOwner.Options): Promise<AdbServerClient.DeviceObserver> {
  ...
  async createTransport(device: AdbServerClient.DeviceSelector): Promise<AdbServerTransport> {
    ...
    transport.disconnected.finally(() => waitAbortController.abort()).catch(() => {});
...
export namespace AdbServerClient {
  ...
  export type ConnectionState = (typeof AdbServerClient.ConnectionState)[number];
