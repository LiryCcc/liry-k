...
export class AdbServerDeviceObserverOwner {
  ...
  async createObserver(options?: AdbServerDeviceObserverOwner.Options): Promise<AdbServerClient.DeviceObserver> {
    options?.signal?.throwIfAborted();

    const onDeviceAdd = new EventEmitter<readonly AdbServerClient.Device[]>();
    const onDeviceRemove = new EventEmitter<readonly AdbServerClient.Device[]>();
    const onListChange = new StickyEventEmitter<readonly AdbServerClient.Device[]>();
    const onError = new StickyEventEmitter<Error>();

    const includeStates = options?.includeStates ?? ['device', 'unauthorized'];
    const observer = {
      includeStates,
      onDeviceAdd,
      onDeviceRemove,
