...
export class Adb implements Closeable {
  ...
  async getProp(key: string): Promise<string> {
    const output = await this.subprocess.noneProtocol.spawn(['getprop', key]).wait().toString();
    return output.trim();
  }

  rm(filenames: string | readonly string[], options?: { recursive?: boolean; force?: boolean }): Promise<string> {
    const args = ['rm'];
...
export namespace Adb {
   ...
   */
  export interface Socket extends ReadableWritablePair<Uint8Array, MaybeConsumable<Uint8Array>>, Closeable {
    get service(): string;

    get closed(): Promise<undefined>;
  }

  export type IncomingSocketHandler = (socket: Socket) => MaybePromiseLike<void>;
}
