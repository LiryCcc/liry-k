...
export class Service {
  ...
  constructor(adb: Adb, options?: ServiceOptions) {
    ...
    this.#needPushMkdirWorkaround = this._adb.canUseFeature(AdbFeature.Shell2) && !this.fixedPushMkdir;
  }
  ...
  async write(options: WriteOptions): Promise<void> {
    if (this.needPushMkdirWorkaround) {
      ...
      await this._adb.subprocess.noneProtocol.spawn(['mkdir', '-p', escapeArg(dirname(options.filename))]).wait();
    }
