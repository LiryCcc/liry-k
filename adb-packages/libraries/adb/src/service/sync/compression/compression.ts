...
export function hasRuntimeSupport(format: Format, mode: Mode) {
  ...
  const Cache = mode === Mode.Compress ? CompressionFormatCache : DecompressionFormatCache;
  if (Cache.has(format)) {
  ...
  try {
    ...
    stream.writable.abort().catch(NOOP);
...
export function canUseBrotli(adb: Adb, mode: Mode) {
  return adb.canUseFeature(AdbFeature.SendReceive2Brotli) && hasRuntimeSupport(Format.Brotli, mode);
}

export function canUseLz4(adb: Adb, mode: Mode) {
  return adb.canUseFeature(AdbFeature.SendReceive2Lz4) && hasRuntimeSupport(Format.Lz4, mode);
}

export function canUseZstd(adb: Adb, mode: Mode) {
  return adb.canUseFeature(AdbFeature.SendReceive2Zstd) && hasRuntimeSupport(Format.Zstd, mode);
}
