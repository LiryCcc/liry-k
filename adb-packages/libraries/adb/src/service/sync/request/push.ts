...
async function pipeFileData(
  ...
  if (compression !== Compression.Format.None) {
    ...
    file.pipeTo(new MaybeConsumable.WrapWritableStream(stream.writable)).catch(NOOP);
...
export function send(options: SendOptions) {
  ...
  if (options.compression !== undefined && options.compression !== Compression.Format.None) {
