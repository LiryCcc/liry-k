...
export async function* v1(pool: SocketPool, path: string): AsyncGenerator<EntryResponse, void, void> {
...
export async function* v2(pool: SocketPool, path: string): AsyncGenerator<EntryV2Response, void, void> {
  const socket = await pool.acquire();
  let completed = false;
  let error: unknown;
