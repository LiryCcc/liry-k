...
describe('md5', () => {
  it('should digest empty string', () => {
    ...
    assert.deepStrictEqual(Buffer.from(actual), Buffer.from(expected, 'hex'));
  });
  ...
  it("should digest 'The quick brown fox jumps over the lazy dog'", () => {
    ...
    const actual = md5Digest(encodeUtf8('The quick brown fox jumps over the lazy dog'));
    assert.deepStrictEqual(Buffer.from(actual), Buffer.from(expected, 'hex'));
  });
  ...
  it('should digest multiple long messages', () => {
    ...
    for (let loop = 0; loop < 3; loop += 1) {
      ...
      assert.deepStrictEqual(Buffer.from(md5.digest()), Buffer.from('b3e98306e7367f93cd7cb870af64f7b7', 'hex'));
    }
  });
});
