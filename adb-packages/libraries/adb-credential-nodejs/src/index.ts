...
import { adbGeneratePublicKey, decodeBase64, decodeUtf8, encodeBase64, rsaParsePrivateKey } from '@yume-chan/adb';
...
export class TangoNodeStorage implements TangoKeyStorage {
  ...
  async save(privateKey: Uint8Array, name: string | undefined): Promise<undefined> {
    ...
    await writeFile(userKeyPath + '.pub', decodeUtf8(encodeBase64(publicKey)) + ' ' + name + '\n', {
      encoding: 'utf8',
      mode: 0o644
    });
  }
  ...
  async #readKey(path: string): Promise<TangoKey> {
    ...
    const name = (await this.#readPublicKeyName(path)) ?? this.#getDefaultName();
    return { privateKey, name };
  }

  async *#readVendorKeys(path: string): AsyncGenerator<MaybeError<TangoKey>, void, void> {
    let stats;
    try {
    ...
    if (stats.isFile()) {
      try {
        ...
        yield wrapError(e, (e) => new VendorKeyError(path, { cause: e }));
        return;
      }
    }
    ...
    if (stats.isDirectory()) {
      ...
      for await (const dirent of dir) {
        ...
        try {
          ...
          yield wrapError(e, (e) => new VendorKeyError(file, { cause: e }));
        }
      }
    }
  }
  ...
  async *load(): AsyncGenerator<MaybeError<TangoKey>, void, void> {
    ...
    if (existsSync(userKeyPath)) {
      try {
        ...
        yield wrapError(e, (e) => new InvalidKeyError(userKeyPath, { cause: e }));
      }
    }
...
export type { TangoKey, TangoKeyStorage, TangoPrfSource } from '@yume-chan/adb-credential-web';
