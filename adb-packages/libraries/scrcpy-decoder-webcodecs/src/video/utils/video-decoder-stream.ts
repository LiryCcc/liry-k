...
function decodeFirstKeyFrame(
  ...
  decoder.decode(
    new EncodedVideoChunk({
      ...
      data: config.raw ? concatBuffers([config.raw, chunk.data]) : chunk.data
    })
  );
}

function decodeFrame(decoder: VideoDecoder, chunk: CodecTransformStream.VideoChunk) {
  decoder.decode(
    new EncodedVideoChunk({
      type: chunk.type,
      timestamp: chunk.timestamp,
...
export class VideoDecoderStream
  extends TransformStream<CodecTransformStream.Config | CodecTransformStream.VideoChunk, VideoFrame>
  implements ScrcpyVideoDecoderPerformanceCounterInterface
{
  ...
  constructor() {
    ...
    super({
      ...
      flush: async () => {
        ...
        if ('decoder' in this.#state && this.#state.decoder.state === 'configured') {
  ...
  #createDecoder(config: CodecTransformStream.Config) {
      ...
      error: (error) => {
        ...
        if (
          ...
          try {
            ...
            this.#skipFramesBefore = this.#state.frames.at(-1)!.timestamp;
    ...
    decoder.addEventListener('dequeue', () => this.#onDequeue.fire(undefined), {
  ...
  #dispose() {
    ...
    if ('decoder' in this.#state && this.#state.decoder.state !== 'closed') {
