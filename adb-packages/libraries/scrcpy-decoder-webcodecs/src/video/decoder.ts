...
import { Av1TransformStream, H264TransformStream, H265TransformStream } from './codec/index.js';
import type { CodecTransformStream } from './codec/type.js';
import type { VideoFrameRenderer } from './render/index.js';
import { AutoCanvasRenderer, BitmapVideoFrameRenderer, RendererController } from './render/index.js';
...
export class WebCodecsVideoDecoder implements ScrcpyVideoDecoder {
  ...
  get type() {
    return this.#rawDecoder.hardwareAcceleration !== 'prefer-software' ? 'hardware' : 'software';
  }
  #onTypeChange = pipe(this.#rawDecoder.onHardwareAccelerationChange, (value) =>
    value !== 'prefer-software' ? 'hardware' : 'software'
  );
  get onTypeChange() {
  ...
  constructor({
    ...
    switch (this.#codec) {
      ...
      case ScrcpyVideoCodecId.AV1:
        ...
        break;
      default:
        throw new Error(`Unsupported codec: ${String(this.#codec)}`);
    }
...
export namespace WebCodecsVideoDecoder {
  export interface Options extends Pick<VideoDecoderConfig, 'hardwareAcceleration' | 'optimizeForLatency'> {
    /**
