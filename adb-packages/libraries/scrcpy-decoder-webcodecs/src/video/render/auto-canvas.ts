...
export class AutoCanvasRenderer implements VideoFrameRenderer {
  ...
  #createInner(options: AutoCanvasRenderer.Options | undefined, webgl: boolean) {
    ...
    if (webgl) {
      ...
      webgl.onContextLost(() => {
        ...
        if (this.#lastFrame) {
          this.#innerWriter.write(this.#lastFrame.clone()).catch((e) => this.#writableController.error(e));
        }
      });
      this.#inner = webgl;
    } else {
...
export namespace AutoCanvasRenderer {
  export interface Options extends Omit<WebGLVideoFrameRenderer.Options, 'canvas'> {
