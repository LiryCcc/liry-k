...
const packages = await Promise.all(
  packageNames.map(async (name) => {
    ...
    const { default: packageJson } = await import(pathToFileURL(packageJsonPath), { with: { type: 'json' } });
...
for (const item of packages) {
  if (!majorBump) {
    const changed = execSync(`git diff --name-only ${lastTag} -- "${item.folder}"`, { encoding: 'utf8' });
  ...
  await writeFile(item.packageJsonPath, JSON.stringify(item.packageJson, undefined, 4) + '\n');
