...
本仓库是一个多语言 monorepo。npm 包任务由 **Turborepo**（`turbo.json`）编排与缓存；Rust / Java 仍分别由 Cargo / Gradle 管理。
...
| 命令                | 说明                                        |
| ------------------- | ------------------------------------------- |
| `pnpm install`      | 安装所有依赖并通过 Turbo 构建 TypeScript 包 |
| `pnpm build`        | `turbo run build`（按依赖图构建并缓存）     |
| `pnpm test`         | `turbo run test`                            |
| `pnpm format`       | Prettier + `turbo run format`               |
