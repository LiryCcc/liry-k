...
import type { StructDeserializer, StructLike, StructSerializeContext, StructSerializer } from './types.js';

export type StructField =
  Field<unknown, string, unknown, unknown> | (StructSerializer<unknown> & StructDeserializer<unknown>);
...
export type FieldOmitInit<T extends StructField> =
  T extends Field<unknown, infer U, unknown, unknown> ? (string extends U ? never : U) : never;
...
export class StructEmptyError extends StructDeserializeError {
  ...
  static override [Symbol.hasInstance](value: unknown) {
    return !!(value as StructEmptyError | undefined)?.[StructEmptyErrorBrand];
  }
...
export type ExtraToIntersection<Extra extends Record<PropertyKey, unknown> | undefined> = Extra extends undefined
  ? unknown
  : Extra;
...
>
  extends StructSerializer<FieldsInit<Fields>>, StructDeserializer<PostDeserialize> {
...
export function struct<
  Fields extends Record<string, Field<unknown, string, Partial<FieldsValue<Fields>>, unknown> | StructLike<unknown>>,
  Extra extends Record<PropertyKey, unknown> | undefined = undefined,
...
>(
  ...
  return {
    ...
    size,
    serialize(source: FieldsInit<Fields>, bufferOrContext?: Uint8Array | StructSerializeContext): Uint8Array | number {
    ...
    deserialize: bipedal(function* (this: Struct<Fields, Extra, PostDeserialize>, then, reader: AsyncExactReadable) {
      const startPosition = reader.position;

      const result = {} as Record<string, unknown>;
      const context: FieldDeserializeContext<Partial<FieldsValue<Fields>>> = {
      ...
      if (options.postDeserialize) {
        return options.postDeserialize.call(result as never, result as never);
      } else {
        return result;
      }
    })
