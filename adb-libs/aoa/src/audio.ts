...
export type AoaAudioMode = (typeof AOA_AUDIO_MODE)[keyof typeof AOA_AUDIO_MODE];
 ...
 */
export const aoaSetAudioMode = async (device: USBDevice, mode: AoaAudioMode) => {
  await device.controlTransferOut(
    {
      recipient: 'device',
      requestType: 'vendor',
      request: AOA_REQUEST_TYPE.SetAudioMode,
      value: mode,
      index: 0
    },
    new ArrayBuffer(0)
...
const findAudioStreamingInterface = (device: USBDevice) => {
 ...
 */
export const aoaGetAudioStream = (device: USBDevice) => {
