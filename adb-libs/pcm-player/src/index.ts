export abstract class PcmPlayer<T> {
  ...
  abstract feedCore(worklet: AudioWorkletNode, source: T): void;
...
export class Int16PcmPlayer extends PcmPlayer<Int16Array> {
  override sourceName = 'int16-source-processor';

  override feedCore(worklet: AudioWorkletNode, source: Int16Array) {
...
export class Float32PcmPlayer extends PcmPlayer<Float32Array> {
  override sourceName = 'float32-source-processor';

  override feedCore(worklet: AudioWorkletNode, source: Float32Array) {
    if (source.byteOffset !== 0 || source.byteLength !== source.buffer.byteLength) {
      source = source.slice();
    }
...
export class Float32PlanerPcmPlayer extends PcmPlayer<Float32Array[]> {
  override sourceName = 'float32-planer-source-processor';
